...
<body onload="document.Paypal.submit();"> 
    ...
    <p>Redirecting to Paypal <img src="images/wait.gif" height="150" width="150"/></p>
   ...
   <!-- This form contains hidden input fields with variable names required by the PayPal Payments Standard API 
    ...
    <form id="Paypal" name="Paypal" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post" >
        ...
        <input type="hidden" name="item_name_<%# Container.ItemIndex + 1 %>" value="<%# Eval("Items") %>" />
        <input type="hidden" name="quantity_<%# Container.ItemIndex + 1 %>" value="<%# Eval("Quantity") %>" />
        <input type="hidden" name="amount_<%# Container.ItemIndex + 1 %>" value="<%# Eval("Price") %>" />
   ...
   <%-- <input type="hidden" name="shipping_1" value="5" />
    ...
    <input type="hidden" name="tax_1" value="3" /> 
    <input type="hidden" name="currency_code" value="USD" />
    <input type="hidden" name="return" value="<%= System.IO.Path.Combine(Request.PhysicalApplicationPath,"Thanks.aspx") %>" />
    <input type="hidden" name="cancel_return" value="<%=System.IO.Path.Combine(Request.PhysicalApplicationPath,"OrderSummary.aspx") %>" />
